...
# 自作モジュールのインポート
from modules.voice_chat import APIVoiceChat
from modules.utils import setup_logging, check_api_key, get_env_path
...
class VoiceChat(APIVoiceChat):
    """
    Gemini APIとStyle-BERT-VITS2 APIを使用したチャットボットクラス
    
    このクラスはAPIVoiceChatを拡張し、コマンドライン対話機能を追加します。
    """
    ...
    def __init__(self, character_name: str = DEFAULT_CHARACTER):
        ...
        # 利用可能なモデルを表示
        models = self.available_models.get("models", [])
        print(f"利用可能な音声モデル:")
        for model in models:
...
現在の設定:
  ...
  分割間隔: {self.split_interval}
    ...
    async def chat_loop(self):
        ...
        チャットループを実行する
        """
        print("\nVerbalis Voice Chat Assistant (API版) を起動しました。")
...
async def main():
    ...
    """
    # コマンドライン引数の解析
    parser = argparse.ArgumentParser(description="Verbalis Voice Chat Assistant (API版)")
